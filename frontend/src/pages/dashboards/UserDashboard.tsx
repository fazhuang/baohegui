 ...
 *
 * 编排层：导入 features/dashboard/components 下的 KpiCard/AnnouncementCard 等独立组件。
 */
...
import MiniTrendBar from '../../features/dashboard/components/MiniTrendBar';
import PassRateRing from '../../features/dashboard/components/PassRateRing';
import AnnouncementCard from '../../features/dashboard/components/AnnouncementCard';
...
interface DashboardData {
  ...
  recent_reports: Array<{
    id: number | string; source_file: string; status: string; risk_level: string;
    created_at: string; risk_level_cn: string;
  }>;
  announcements: Array<{
    id: number; title: string; severity: string; category: string;
    case_date: string; summary: string; source: string;
  }>;
      ...
      try {
        ...
        setData({
          ...
          announcements: (annResp || []).slice(0, 5).map((a: Record<string, unknown>) => ({
            id: a.id as number, title: a.title as string, severity: a.severity as string,
            category: a.category as string, case_date: a.case_date as string,
            summary: a.summary as string, source: a.source as string,
          })),
