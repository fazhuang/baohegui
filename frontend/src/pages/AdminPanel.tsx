 ...
 *
 * 编排层：使用 features/admin/hooks 的状态管理。
 */
...
import {
  deleteUser,
  compareFiles, listReports,
} from '../services/api'
import type { CompareResult, UserInfo } from '../types/admin-types'
import type { BillingStatus } from '../types'
import { getErrorMessage } from '../utils/error'
import { useUserManage, useAuditLog, useBilling } from '../features/admin/hooks'
...
const CompareTab: React.FC = () => {
  ...
  useEffect(() => {
    listReports().then(data => {
      ...
      const initialA = cmpSearchParams.get('a'); const initialB = cmpSearchParams.get('b')
      if (initialA) setSelectedA(Number(initialA)); if (initialB) setSelectedB(Number(initialB))
    }).catch(() => {})
  }, [cmpSearchParams])
  ...
  const fileOptions = reports.map(r => ({
    value: r.file_id, label: `#${r.file_id} — 评分 ${r.total_score} (${r.created_at?.slice(0, 10) || '?'})`,
  }))
            ...
            <Card size="small" title="合规评分变化">
              <Row gutter={[16, 12]}>
                {Object.entries(result.score_diff).map(([key, val]) => {
                  const delta = val.delta ?? 0; const color = delta > 0 ? '#16a34a' : delta < 0 ? '#dc2626' : '#6b7280'
                  return (
                    <Col span={12} key={key}>
                      <Statistic title={key} value={val.b}
                        suffix={delta !== 0 ? <Text style={{ color, fontSize: 14 }}>({delta > 0 ? '+' : ''}{delta})</Text> : undefined}
                        valueStyle={{ fontSize: 20 }} />
                      <Progress percent={val.b} size="small" strokeColor={val.b >= 60 ? '#16a34a' : '#eab308'} />
                    </Col>
                  )
                })}
              </Row>
